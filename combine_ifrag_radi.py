...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Combine iFrag, conservation, raDI, and optional blastPDB structural anchors into a docking-oriented interface score."
    )
...
def run_conservation(
    ...
    return load_optional_json(out_dir / "conservation_summary.json")


def can_reuse_conservation_prepared_dir(conservation_summary: dict[str, object], conservation_out: Path) -> bool:
    if str(conservation_summary.get("status") or "") == "no_evidence":
        return False
    required_paths = (
        conservation_out / "pairable_pairs_sequence_backed.tsv",
        conservation_out / "chain1_famsa.fasta",
        conservation_out / "chain2_famsa.fasta",
        conservation_out / "chain1_trimmed.fasta",
        conservation_out / "chain2_trimmed.fasta",
    )
...
def main() -> None:
    ...
    conservation_summary: dict[str, object] = {}
    if needs_conservation:
        ...
        conservation_summary = run_conservation(
            project_root,
            query1.fasta_path,
            query2.fasta_path,
            conservation_out,
    ...
    conservation_prepared_dir = (
        conservation_out
        if needs_conservation and can_reuse_conservation_prepared_dir(conservation_summary, conservation_out)
        else None
    )
    if needs_radi:
        if query1_search_tsv is None or query2_search_tsv is None:
    ...
    if needs_conservation and not conservation_summary:
        conservation_summary = load_optional_json(conservation_out / "conservation_summary.json")
